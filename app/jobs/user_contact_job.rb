class UserContactJob < ApplicationJob
  queue_as :user_contact
  require 'mail'

  def perform(mail_server_id,from_email,user_email,subject,html,text)
  	require 'mail'
    mail_server = MailServer.find(mail_server_id)
  	...
  	send_mail(user_email,from_email,subject,html,text)
	end
